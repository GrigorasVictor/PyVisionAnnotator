...
\begin{equation}
    \text{detectie}_i =
    (\text{eticheta}_i, \text{scor}_i, \text{dreptunghi}_i, \text{masca}_i).
    \label{eq:detectie-autoseg}
\end{equation}
Ecuația~\ref{eq:detectie-autoseg} descrie structura unei detecții generate de AutoSeg. Pentru detecția cu numărul $i$, câmpul \textit{eticheta} indică numele clasei, \textit{scor} indică nivelul de încredere, \textit{dreptunghi} localizează obiectul printr-o cutie de încadrare, iar \textit{masca} descrie conturul obiectului atunci când modelul furnizează segmentare.
Dreptunghiul localizează rapid obiectul, iar masca descrie conturul său.
...
Această structură este ilustrată în figura~\ref{fig:jwt-structure} (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.82\textwidth]{figs/jwt-diagram.png}
    \caption{Structura generală a unui token JWT (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).}
