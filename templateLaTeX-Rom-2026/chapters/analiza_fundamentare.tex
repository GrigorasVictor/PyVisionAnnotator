...
O imagine de intrare poate fi descrisă prin mulțimea adnotărilor asociate ei:
\begin{equation}
    \text{adnotari}(\text{imagine}) =
    \{\text{adnotare}_1, \text{adnotare}_2, \ldots, \text{adnotare}_n\}.
    \label{eq:multime-adnotari}
\end{equation}
Ecuația~\ref{eq:multime-adnotari} arată că pentru o imagine există o colecție de adnotări individuale, notate explicit ca \textit{adnotare 1}, \textit{adnotare 2} până la \textit{adnotare n}. Fiecare element din această colecție reprezintă un obiect sau o regiune marcată de utilizator ori propusă de un model extern.
...
AutoSeg este algoritmul folosit pentru detecție și generare de contururi/poligoane la nivelul întregii imagini.
Intrările sale sunt imaginea curentă, lista de clase urmărite și un prag minim de încredere.
Ieșirea este o listă de obiecte candidate, fiecare obiect putând conține o etichetă, un scor, o cutie de încadrare și un contur folosit ca poligon editabil.
...
\begin{enumerate}
    ...
    \item pentru fiecare predicție validă se extrage dreptunghiul de încadrare și, dacă există, conturul obiectului;
...
\begin{equation}
    \text{detectie}_i =
    (\text{eticheta}_i, \text{scor}_i, \text{dreptunghi}_i, \text{contur}_i).
    \label{eq:detectie-autoseg}
\end{equation}
Ecuația~\ref{eq:detectie-autoseg} descrie structura unei detecții generate de AutoSeg. Pentru detecția cu numărul $i$, câmpul \textit{eticheta} indică numele clasei, \textit{scor} indică nivelul de încredere, \textit{dreptunghi} localizează obiectul printr-o cutie de încadrare, iar \textit{contur} descrie punctele care pot fi transformate într-un poligon editabil.
Dreptunghiul localizează rapid obiectul, iar conturul descrie forma sa aproximativă.
Dacă un contur nu este disponibil, dreptunghiul de încadrare rămâne o reprezentare minimă utilă.
...
Cel mai simplu indiciu este un punct ales în interiorul regiunii de interes.
Modelul returnează o mască asociată acelei regiuni:
\begin{equation}
    \text{AutoMask}(\text{imagine}, \text{punct selectat})
    \rightarrow \text{masca generata}.
    \label{eq:automask-punct}
\end{equation}
Ecuația~\ref{eq:automask-punct} exprimă fluxul de segmentare interactivă: utilizatorul oferă imaginea și un punct selectat pe obiect, iar modelul produce o mască pentru regiunea corespunzătoare acelui punct.

În modul de segmentare pe întreaga imagine, algoritmul poate produce mai multe măști candidate:
\begin{equation}
    \text{AutoMask}(\text{imagine})
    \rightarrow \{\text{masca}_1, \text{masca}_2, \ldots, \text{masca}_m\}.
    \label{eq:automask-complet}
\end{equation}
Ecuația~\ref{eq:automask-complet} descrie cazul în care segmentarea nu pornește de la un singur punct, ci de la întreaga imagine. Rezultatul este o listă de măști candidate, notate ca \textit{masca 1}, \textit{masca 2} până la \textit{masca m}.
...
\begin{equation}
    P(\text{token}_n \mid
    \text{token}_1, \text{token}_2, \ldots, \text{token}_{n-1}).
    \label{eq:predictie-token-llm}
\end{equation}
Ecuația~\ref{eq:predictie-token-llm} arată că modelul calculează probabilitatea următorului token pe baza tokenilor deja existenți în conversație. Cu alte cuvinte, răspunsul nu este generat dintr-o singură operație, ci prin alegerea repetată a următorului fragment de text.
...
În aplicație sunt folosite două moduri locale de rulare: llama.cpp, prin llama-cpp-python, și Ollama, prin client local.
...
\begin{equation}
    \text{stare}_{v+1} =
    \text{aplica}(\text{stare}_v, \text{eveniment primit}).
    \label{eq:actualizare-stare-colaborativa}
\end{equation}
Ecuația~\ref{eq:actualizare-stare-colaborativa} explică actualizarea unei sesiuni colaborative. Starea curentă a sesiunii, notată ca \textit{stare v}, este modificată prin aplicarea unui eveniment primit, iar rezultatul devine \textit{stare v+1}, adică următoarea versiune a sesiunii.
...
Algoritmii descriși anterior sunt susținuți de trei familii de modele: YOLO pentru detecție și propuneri de contur, Mask2Former pentru segmentare externă și modele lingvistice bazate pe Transformer pentru asistența locală.
...
În implementarea proiectului, fiecare rezultat YOLOE este consumat ca dreptunghi de încadrare sau ca poligon.
...
Această capacitate teoretică de segmentare nu înseamnă că fluxul AutoSeg din proiect creează direct măști; scriptul curent produce box-uri și coordonate de contur, iar clientul le transformă în BoundingBoxItem sau PolygonItem.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figs/yoloe.jpg}
    \caption{Arhitectura YOLOE folosită ca reper pentru AutoSeg (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).}
    \label{fig:yoloe-architecture}
\end{figure}

În figura~\ref{fig:yoloe-architecture} (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}), se observă separarea dintre extragerea de caracteristici, agregarea multi-scală și capetele de predicție.
...
\begin{figure}[!htbp]
    ...
    \caption{Arhitectura Mask2Former pentru predicția măștilor (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).}
    \label{fig:mask2former-architecture}
\end{figure}

În figura~\ref{fig:mask2former-architecture} (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}), masca nu este doar un rezultat final, ci și un element folosit în atenția modelului.
...
\begin{figure}[!htbp]
    ...
    \caption{Structură generală de model Transformer de tip decoder-only (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).}
    \label{fig:transformer-decoder-only}
\end{figure}

În figura~\ref{fig:transformer-decoder-only} (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}), blocurile principale sunt embedding-ul pozițional, atenția mascată, normalizarea și rețeaua feed-forward.
...
llama.cpp permite încărcarea directă a unui model local și controlul parametrilor de inferență~\cite{llamacpp2026}.
...
Fluxul este ilustrată în figura~\ref{fig:jwt-structure} (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figs/jwt-diagram.png}
    \caption{Fluxul de autentificare și propagare a tokenului JWT (\hyperref[tab:surse-figuri-ai]{Tabelul A.1}).}
...
Pentru AutoSeg, rezultatul înseamnă box-uri și poligoane.
Pentru AutoMask, rezultatul înseamnă măști editabile.
Aceeași structură este folosită și pentru LLM-uri: Ollama și llama.cpp sunt apelate local, iar răspunsurile lor sunt livrate în interfață prin worker-e.
...
Această decizie păstrează aplicația receptivă chiar și când un model extern sau o cerere de rețea durează mai mult.
...
\subsection{Java Spring Boot pentru serviciile backend}
...
De exemplu, rutele de adnotare sunt expuse sub prefixul public /annotation, în timp ce serviciile interne rulează pe porturi separate.
...
În proiect există worker-e pentru autentificare, chat STOMP, colaborare STOMP, AutoSeg, AutoMask, Ollama și llama.cpp.
...
Poate folosi AutoMask pentru o regiune indicată prin click sau pentru segmentare pe întreaga imagine.
