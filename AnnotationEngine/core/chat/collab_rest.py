...
class CollabRestClient:
    ...
    def _json_request(self, method: str, path: str) -> tuple[int, dict[str, Any]]:
        req = request.Request(
    ...
    def upload_temp_image(
        ...
        *,
        ...
        req = request.Request(
            url=f"{self._base_url}{COLLAB_UPLOAD_TEMP_PATH}",
            method="POST",
            data=payload,
            headers={
                "Authorization": f"Bearer {self._jwt_token}",
    ...
    def download_temp_image(self, image_id: str, token: str) -> tuple[int, bytes, str]:
        ...
        url = f"{self._base_url}{COLLAB_MEDIA_TEMP_PREFIX}/{iid}?{q}"
        req = request.Request(
            url=url,
            method="GET",
            headers={
